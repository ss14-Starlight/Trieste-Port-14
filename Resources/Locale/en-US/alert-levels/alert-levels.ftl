alert-level-announcement = Attention! Platform alert level is now {$name}! {$announcement}

alert-level-unknown = Unknown.
alert-level-unknown-instructions = Unknown.

alert-level-green = Green
alert-level-green-announcement = It is now safe to return to your daily activities.
...
alert-level-violet-announcement = There is a viral threat on the platform. Medical staff are advised to isolate citizens with any symptoms. Citizens are advised to distance themselves from others and perform safety measures to prevent further spread.
...
alert-level-yellow-announcement = There is a structural or atmospheric threat within the platform. Engineering staff are advised to immediately respond and perform safety measures. Citizens are advised to stay away from the threat, and stay in their workplaces if necessary.
